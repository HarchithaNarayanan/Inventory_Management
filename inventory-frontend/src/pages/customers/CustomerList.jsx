...
const CustomerList = () => {
  ...
  const fetchCustomers = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/api/customers/page`, {
  ...
  return (
    <div className="page-container">
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h1 style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-text-primary)' }}>Customers</h1>
        <button className="btn-primary" style={{ width: 'auto', marginTop: 0 }} onClick={() => navigate('/customers/new')}>
          + Add Customer
        </button>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid #e2e8f0', textAlign: 'left', color: 'var(--color-text-secondary)' }}>
                ...
                <th style={{ padding: '12px' }}>Credit Amount</th>
                <th style={{ padding: '12px' }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {data.map(customer => (
                <tr key={customer.customerId} style={{ borderBottom: '1px solid #e2e8f0' }}>
                  <td style={{ padding: '12px', fontWeight: '500' }}>{customer.customerCode}</td>
                  <td style={{ padding: '12px' }}>{customer.customerName}</td>
                  <td style={{ padding: '12px' }}>{customer.city || '-'}</td>
                  <td style={{ padding: '12px' }}>{customer.phoneNo || '-'}</td>
                  <td style={{ padding: '12px', color: '#059669', fontWeight: '600' }}>
                    ₹ {customer.creditAmount?.toFixed(2) || '0.00'}
                  </td>
