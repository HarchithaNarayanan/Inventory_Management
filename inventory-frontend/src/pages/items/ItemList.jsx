...
const ItemList = () => {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/api/items/page`, {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setData(data.filter(i => i.itemId !== id));
    } catch (err) {
  ...
  return (
    <div className="page-container">
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h1 style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-text-primary)' }}>Items / Products</h1>
        <button className="btn-primary" style={{ width: 'auto', marginTop: 0 }} onClick={() => navigate('/items/new')}>
          + Add Item
        </button>
      </div>
      ...
      <div className="table-card" style={{ background: 'var(--color-card-bg)', borderRadius: 'var(--radius-lg)', padding: '24px', boxShadow: 'var(--shadow-card)', border: '1px solid var(--color-card-border)' }}>
        {data.length === 0 ? (
          <p style={{ color: 'var(--color-text-secondary)', textAlign: 'center', padding: '20px' }}>
            {search ? 'No items match your search.' : 'No items found. Click "Add Item" to create one.'}
          </p>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid #e2e8f0', textAlign: 'left', color: 'var(--color-text-secondary)' }}>
                ...
                <th style={{ padding: '12px' }}>Purchase Rate</th>
                <th style={{ padding: '12px' }}>Selling Rate</th>
            ...
            <tbody>
              {data.map(item => (
                <tr key={item.itemId} style={{ borderBottom: '1px solid #e2e8f0' }}>
                  <td style={{ padding: '12px', fontWeight: '500' }}>{item.itemCode}</td>
                  <td style={{ padding: '12px' }}>{item.itemName}</td>
                  <td style={{ padding: '12px' }}>
                    <span style={{ padding: '4px 8px', background: '#f3e8ff', color: '#9333ea', borderRadius: '4px', fontSize: '12px', fontWeight: '600' }}>
                      {item.category || 'GENERAL'}
                    </span>
                  </td>
                  <td style={{ padding: '12px', fontWeight: '600', color: item.openingStock <= item.reorderLevel ? 'var(--color-error)' : 'inherit' }}>
                    {item.openingStock?.toFixed(2)} {item.unitOfMeasure}
                  </td>
                  <td style={{ padding: '12px' }}>₹ {item.purchaseRate?.toFixed(2) || '0.00'}</td>
                  <td style={{ padding: '12px' }}>₹ {item.sellingRate?.toFixed(2) || '0.00'}</td>
                  <td style={{ padding: '12px' }}>
                    <button
