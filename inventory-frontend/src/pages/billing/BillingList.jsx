...
const BillingList = () => {
  ...
  const fetchBillings = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/api/billings/page`, {
  ...
  return (
    <div className="page-container">
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h1 style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-text-primary)' }}>Sales (Billing)</h1>
        <button className="btn-primary" style={{ width: 'auto', marginTop: 0 }} onClick={() => navigate('/billing/new')}>
          + New Bill
        </button>
      </div>
      ...
      <div className="table-card" style={{ background: 'var(--color-card-bg)', borderRadius: 'var(--radius-lg)', padding: '24px', boxShadow: 'var(--shadow-card)', border: '1px solid var(--color-card-border)' }}>
        {data.length === 0 ? (
          <p style={{ color: 'var(--color-text-secondary)', textAlign: 'center', padding: '20px' }}>
            {search ? 'No bills match your search.' : 'No bills found. Click "New Bill" to create one.'}
          </p>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {data.map(bill => (
                <tr key={bill.billingId} style={{ borderBottom: '1px solid #e2e8f0' }}>
                  <td style={{ padding: '12px', fontWeight: '500' }}>{bill.billNo}</td>
                  <td style={{ padding: '12px' }}>{bill.billDate}</td>
                  <td style={{ padding: '12px' }}>{bill.customerName}</td>
                  <td style={{ padding: '12px' }}>₹{bill.netAmount.toFixed(2)}</td>
                  <td style={{ padding: '12px' }}>
                    <span style={{ padding: '4px 8px', background: bill.paymentStatus === 'PAID' ? '#dcfce7' : '#fee2e2', color: bill.paymentStatus === 'PAID' ? '#166534' : '#991b1b', borderRadius: '4px', fontSize: '12px', fontWeight: '600' }}>
                      {bill.paymentStatus}
                    </span>
                  </td>
                  <td style={{ padding: '12px' }}>
