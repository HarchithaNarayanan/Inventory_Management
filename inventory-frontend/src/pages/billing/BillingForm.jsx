...
const BillingForm = () => {
  ...
  const handleDetailChange = (index, field, value) => {
    ...
    if (field === 'itemId') {
      ...
      if (selectedItem) {
        newDetails[index].unitPrice = selectedItem.sellingRate || 0;
  ...
  const addLineItem = () => {
    setBillingDetails([...billingDetails, { itemId: '', quantity: 1, unitPrice: 0, gstPercent: 0, discountPercent: 0 }]);
  };

  const removeLineItem = (index) => {
    const newDetails = billingDetails.filter((_, i) => i !== index);
    setBillingDetails(newDetails);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...formData,
        billingDetails: billingDetails.map(d => ({
  ...
  return (
    <div className="page-container">
      <div className="page-header" style={{ marginBottom: '24px' }}>
        <h1 style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-text-primary)' }}>New Sales Bill</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="form-card" style={{ background: 'var(--color-card-bg)', borderRadius: 'var(--radius-lg)', padding: '24px', boxShadow: 'var(--shadow-card)', border: '1px solid var(--color-card-border)' }}>
          ...
          <tbody>
            {billingDetails.map((detail, index) => (
              <tr key={index}>
                ...
                <td style={{ padding: '8px' }}>
                  <input type="number" className="form-control" value={detail.discountPercent} onChange={e => handleDetailChange(index, 'discountPercent', e.target.value)} min="0" step="0.01" />
                </td>
                <td style={{ padding: '8px' }}>
        ...
        <div className="form-actions" style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
          <button type="button" className="btn-secondary" onClick={() => navigate('/billing')} disabled={loading}>Cancel</button>
