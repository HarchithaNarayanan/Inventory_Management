...
const CompanyForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        await api.put(`/api/companies/${id}`, formData);
      } else {
      ...
      <div className="form-card" style={{ background: 'var(--color-card-bg)', borderRadius: 'var(--radius-lg)', padding: '32px', boxShadow: 'var(--shadow-card)', border: '1px solid var(--color-card-border)' }}>
        <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
          ...
          <div className="form-group">
            ...
            <input type="text" name="companyCode" value={formData.companyCode} onChange={handleChange} className="form-input" required disabled={isEditMode} />
          </div>
          ...
          <div className="form-group">
            ...
            <input type="text" name="website" value={formData.website} onChange={handleChange} className="form-input" />
          </div>
          ...
          <div className="form-group">
            ...
            <input type="text" name="financialYear" value={formData.financialYear} onChange={handleChange} className="form-input" />
          </div>

          <div className="form-group">
            <label className="form-label">Logo URL</label>
            <input type="text" name="logoUrl" value={formData.logoUrl} onChange={handleChange} className="form-input" />
          </div>
