...
const SupplierList = () => {
  ...
  const fetchSuppliers = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/api/suppliers/page`, {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setData(data.filter(s => s.supplierId !== id));
    } catch (err) {
  ...
  return (
    <div className="page-container">
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h1 style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-text-primary)' }}>Suppliers</h1>
        <button className="btn-primary" style={{ width: 'auto', marginTop: 0 }} onClick={() => navigate('/suppliers/new')}>
          + Add Supplier
        </button>
      </div>
      ...
      <div className="table-card" style={{ background: 'var(--color-card-bg)', borderRadius: 'var(--radius-lg)', padding: '24px', boxShadow: 'var(--shadow-card)', border: '1px solid var(--color-card-border)' }}>
        {data.length === 0 ? (
          <p style={{ color: 'var(--color-text-secondary)', textAlign: 'center', padding: '20px' }}>
            {search ? 'No suppliers match your search.' : 'No suppliers found. Click "Add Supplier" to create one.'}
          </p>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {data.map(supplier => (
                <tr key={supplier.supplierId} style={{ borderBottom: '1px solid #e2e8f0' }}>
                  <td style={{ padding: '12px', fontWeight: '500' }}>{supplier.supplierCode}</td>
                  ...
                  <td style={{ padding: '12px' }}>
                    <span style={{ padding: '4px 8px', background: '#e0f2fe', color: '#0284c7', borderRadius: '4px', fontSize: '12px', fontWeight: '600' }}>
                      {supplier.type || 'UNKNOWN'}
                    </span>
                  </td>
                  <td style={{ padding: '12px' }}>
                    <button
                      onClick={() => navigate(`/suppliers/edit/${supplier.supplierId}`)}
